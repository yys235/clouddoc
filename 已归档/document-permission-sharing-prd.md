...
- 关键写操作已增加前后端防重复提交保护：前端 mutating API 会自动生成 `X-CloudDoc-Submission-Key` 并阻止同一请求 in-flight 重复提交；后端对同一会话、同一路径、同一提交 key 的并发写请求返回 409，避免后端卡顿时重复创建、重复删除或重复修改。
- 文件夹工作台和权限/分享弹窗的后端交互按钮已补充 loading/disabled 状态，提交中不可重复点击。
