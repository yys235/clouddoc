...
- 基础画板：独立画板文档类型，支持矩形、圆形、菱形、文本、连接线、平移、缩放、自动保存
- 评论、回复、通知
- 用户注册、登录、组织与成员
- 文档权限：私有 / 公开
...
CloudDoc 支持独立的 `board` 画板文档类型。你可以在“新建文档”弹窗中选择“基础画板”，创建后仍然使用 `/docs/{id}` 打开，但页面会按文档类型自动进入画板编辑器。

当前画板能力：
- 支持文本、矩形、圆角矩形、圆形、菱形和连接线。
- 支持选择、拖拽移动对象、平移画布、缩放画布、撤销/重做、删除对象。
- 支持选中对象后修改填充色和边框色。
- 支持自动保存和手动保存，刷新后继续编辑。
- 复用现有文档权限；无编辑权限时只能查看和平移缩放。

当前边界：
- 暂不支持多人协作。
- 暂不支持把画板嵌入普通文档。
- 暂不支持 SVG/PDF 导出、复杂图形库和画板对象评论。
