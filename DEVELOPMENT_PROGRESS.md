...
2026-05-08 16:27 CST: refreshed the PostgreSQL baseline initialization script `apps/api/sql/001_init.sql` to match the current backend schema. The script now includes folder-tree support tables (`tree_shortcuts`, `user_tree_pins`, `folder_favorites`), AI/Open API integration tables (`integrations`, `integration_tokens`, `integration_resource_scopes`, `integration_audit_logs`, `integration_webhooks`, `integration_webhook_deliveries`, `oauth_authorization_codes`, `oauth_refresh_tokens`), updated indexes, and a safe re-runnable `documents.current_version_id` foreign-key guard. README deployment steps now mention how to manually initialize an empty database with the baseline SQL while retaining automatic runtime schema bootstrapping. Verification: executed `001_init.sql` through SQLAlchemy/psycopg inside a rolled-back transaction => passed; targeted API regression => 3 passed; full API suite => 55 passed; `git diff --check` => passed.
2026-05-08 16:32 CST: added the local PRD `已归档/system-initialization-onboarding-prd.md` for first-deployment system initialization and synchronized it to online CloudDoc document `e4df8b3b-0f47-43a5-8cd8-fbf8402eb978` under `clouddoc / 产品需求`. The PRD defines empty-database setup mode, bootstrap status/initialize APIs, first super-admin creation, default organization/space/folder creation, security policy choices, demo-data separation, setup token protection, initialization access locking, system settings/audit tables, `/setup` UI flow, implementation phases, risks, and backend/frontend/browser test plans. README now links this PRD in the documentation index.
2026-05-08 17:58 CST: implemented the first-deployment system initialization guide from `已归档/system-initialization-onboarding-prd.md`. Added `system_settings` and `system_audit_logs`, bootstrap status/initialize APIs, initialization access middleware returning `423 system_not_initialized`, production-safe demo seed behavior, setup token support, first super-admin/default organization/default space/default `newdoc` and `clouddoc` folder creation, optional sample-document import, and a six-step `/setup` Web wizard. Updated the SQL baseline and runtime schema patch for the new system tables. Verification: `apps/api .venv/bin/python -m compileall app` => passed; `apps/api .venv/bin/pytest tests/test_system_bootstrap_api.py -q` => 3 passed; full API suite => 58 passed; `apps/web npm run build` => passed; `001_init.sql` transaction execution/rollback => passed; Chrome/Playwright smoke for `/setup` and `/documents` => passed; `git diff --check` => passed. Restored local 3100 dev server after build.
2026-05-08 18:42 CST: completed the remaining first-deployment initialization enhancements. Added `python -m app.cli init-system` for unattended deployments, refactored Web and CLI initialization to share the same system-creation service, added the super-admin-only `GET /api/system/settings` summary endpoint, and surfaced initialization state/policies/recent system audit logs on the personal settings page for super admins. README now documents CLI initialization and admin summary visibility; `已归档/system-initialization-onboarding-prd.md` implementation status was updated. Verification: `apps/api .venv/bin/python -m compileall app` => passed; `apps/api .venv/bin/pytest tests/test_system_bootstrap_api.py -q` => 4 passed; full API suite => 59 passed; `apps/web npm run build` => passed.
2026-05-08 18:49 CST: restored local development runtime after the production build. The stable state is API `http://127.0.0.1:8000/health` returning `{"status":"ok"}` and Web `http://127.0.0.1:3100/documents` returning 200. Browser smoke opened `http://127.0.0.1:3100/settings` and confirmed the personal settings page plus super-admin system initialization summary render without page errors.
2026-05-18 11:06 CST: added the beginner-facing AI usage guide `已归档/cloud-doc-ai-user-guide.md`. The guide explains current CloudDoc AI-related capabilities, MCP/Open API/PAT usage, how to start the MCP service, how to configure an AI client with the Streamable HTTP endpoint, permission boundaries, recommended workflows for ordinary documents and board documents, common prompts, and FAQ-style troubleshooting for non-technical users. Sync: created online CloudDoc document `8f4615ce-b7da-42ae-9009-de79d0f4b692` under the existing `clouddoc / 技术架构与 MCP` folder; verification fetched the online document as Markdown and confirmed marker `CloudDoc 当前的 AI 能力不是内置聊天机器人`.
2026-05-18 11:31 CST: fixed the deployed board crash `Uncaught TypeError: crypto.randomUUID is not a function`. Root cause: board/editor code called `crypto.randomUUID()` directly; this browser API requires a secure context and can be missing when CloudDoc is deployed behind plain `http://IP` instead of localhost/HTTPS. Added `apps/web/lib/client-id.ts` as the unified client ID helper, using `crypto.randomUUID()` when available, falling back to `crypto.getRandomValues()` UUID v4 generation, then a timestamp/random fallback. Replaced direct calls in the board and document editors. Verification: `rg "crypto\\.randomUUID\\(\\)" apps/web` => no matches; fallback UUID script without `randomUUID` => generated a valid UUID; `apps/web npm run build` => passed.
2026-05-18 11:39 CST: fixed board shape placeholder editing. Root cause: new shapes stored the Chinese placeholder `输入文本` as real node text, so double-click editing required users to manually delete it. Non-table board nodes now store an empty string by default, render `输入文本` as a muted placeholder when text is empty, and treat legacy stored `输入文本` as placeholder text when entering edit mode. Verification: `apps/web npm run build` => passed; Playwright with local Chrome logged in, created temporary board `pytest-placeholder-*`, inserted a rectangle, double-clicked the shape, and confirmed the editor textarea value was `""` while placeholder was `输入文本`; the temporary board document `058e6325-e375-4909-aadf-9fb833a6f2a0` was deleted after verification.
2026-05-18 11:56 CST: added direct horizontal text-position controls for board shapes. The selected-shape floating toolbar now exposes `文本左对齐`, `文本居中`, and `文本右对齐` buttons next to the font-size control, backed by the existing `style.textAlign` field so read mode, edit mode, and persisted board JSON use the same alignment value. Table cell selections reuse the same buttons and continue to update cell-level `style.textAlign`. Verification: a Playwright red check first confirmed the toolbar lacked the direct alignment buttons; after implementation `apps/web npm run build` passed, then Playwright with local Chrome created temporary board `pytest-align-green-*`, inserted and selected a rectangle, confirmed the toolbar titles included all three alignment buttons, clicked left/right alignment, and verified SVG `text-anchor` changed to `start` and `end`; the temporary board document `281b2ff7-18ad-4cdf-af3f-dd52307f9069` was deleted after verification.
2026-05-18 12:05 CST: fixed board text-box styling and expansion behavior. Text nodes now render their own `style.fill`, `style.stroke`, `strokeWidth`, and dash style instead of always forcing a transparent fill/selection-only stroke, so the existing floating-toolbar fill/border/text color tools work on inserted text boxes. Text nodes are also excluded from connector anchor rendering and quick-add expansion handles, while resize handles remain available. Verification: a Playwright red check confirmed text boxes still showed 4 quick-add handles and 4 connector anchors; after implementation `apps/web npm run build` passed, then Playwright with local Chrome created temporary board `pytest-textbox-green-*`, inserted a text box, verified toolbar style tools were present, verified quick-add and anchor counts were both `0`, changed fill to `#ef4444`, and confirmed the text-box rect fill updated visually; temporary board document `7d7a9432-0d08-40d6-bf57-1b11e95e967b` was deleted after verification.
2026-05-18 16:20 CST: implemented board Ctrl+mouse-wheel zoom and fixed board save failures caused by invalid connector waypoints. The board SVG now handles `Ctrl + wheel` zoom with the mouse position as the anchor point, keeps normal wheel behavior unchanged, clamps zoom to `0.25 ~ 4`, and only marks the board dirty when the user has edit permission. Board saving now normalizes every non-straight connector before submission: valid orthogonal paths are preserved, while missing or diagonal waypoints are regenerated through the existing orthogonal router so the backend no longer rejects them with `Invalid board content`. `updateDocumentContent` now preserves backend error details so board save failures can show a more actionable message. Verification: `apps/web npm run build` => passed; `apps/api .venv/bin/pytest tests/test_documents_api.py -k board -q` => passed; Playwright with local Chrome confirmed Ctrl+wheel changed zoom from `100%` to `108%`, plain wheel did not change zoom, then injected a backend-invalid diagonal connector into a temporary board and confirmed clicking Save returned 200 and persisted an axis-aligned connector path. Temporary test board documents were deleted after verification.
2026-05-18 16:30 CST: strengthened the board save sanitizer after a real save still showed `连接线数据无效，已尝试自动修正后请重试`. Root cause: the previous sanitizer only repaired diagonal waypoints on connectors whose endpoints still existed, but a board can also contain stale connectors that reference deleted/missing nodes, and those make the backend reject the entire board even if visible connectors look valid. Save normalization now filters connectors with missing endpoint nodes, normalizes connector styles, clears straight-connector waypoints, repairs invalid non-straight paths through the existing router, and falls back to a guaranteed orthogonal dogleg if the router output is still not axis-aligned. Verification: `apps/web npm run build` => passed; `apps/api .venv/bin/pytest tests/test_documents_api.py -k board -q` => passed; Playwright injected a board shaped like the reported payload with one valid connector, one diagonal connector, and one stale missing-node connector, then confirmed Save returned 200, the stale connector was removed, and the diagonal connector was persisted as an axis-aligned path.
2026-05-18 16:46 CST: fixed board connector rendering so invalid or slightly non-orthogonal waypoint data cannot appear as a tilted line on screen before save. Root cause: save-time sanitization had been added, but the SVG path generation path could still receive non-axis-aligned points and render a diagonal `L` segment, especially from legacy/stale connector waypoints during page display. `connectorGeometry`, `connectorPath`, and `roundedConnectorPath` now all enforce an orthogonal point sequence before producing SVG path data; rounded connectors keep the intended corner curve, but every explicit line segment is horizontal or vertical. Verification: `apps/web npm run build` => passed; `apps/api .venv/bin/pytest tests/test_documents_api.py -k board -q` => passed; Playwright injected a diagonal connector into a temporary board and confirmed the rendered SVG path had only axis-aligned `L` segments (`M 329 181 L 329 178.5 Q 329 176 331.5 176 L 470 176`).
2026-05-18 17:08 CST: implemented local-first offline draft persistence for document editing. Ordinary doc and board edits now write a pending draft into `localStorage` before attempting server save; successful server save clears only the matching local draft signature so edits made during an in-flight request are not accidentally discarded. If save fails because the backend is unavailable, the UI reports that the draft is saved locally and schedules retry attempts. Reloading a document restores the local unsynced draft first, then uploads it once the backend is reachable. Added a backend-unavailable fallback route for `/docs/[docId]`: when the server cannot fetch the document but the browser has a local draft for that docId, the same document/board editor opens from local storage instead of dropping the user's changes. Verification: `apps/web npm run build` => passed; `apps/api .venv/bin/pytest tests/test_documents_api.py -k board -q` => passed; Playwright verified ordinary docs and board docs restore local drafts after reload, sync them to the backend, and clear localStorage after success; Playwright also stopped the API service, reloaded a board document, and confirmed the local draft rendered while the backend was down, then the API service was restarted and the temporary test document was deleted.
2026-05-19 09:56 CST: fixed the board frontend connector sanitizer regression where small endpoint-height differences could still become diagonal connector data. Root cause: the visual path cleanup step `collapseShortOrthogonalJogs` collapsed tiny H-V-H/V-H-V jogs as an optimization, but for non-straight connectors with empty waypoints this could turn a required orthogonal dogleg back into a diagonal segment before saving. `simplifyOrthogonalPath` now verifies the optimized result is still axis-aligned and falls back to strict orthogonalization if not; save-time fallback waypoint generation also avoids short-jog collapsing and rechecks the final path before submission. Verification: `apps/web npm run build` => passed; `apps/api .venv/bin/pytest tests/test_documents_api.py -k board -q` => passed; direct backend validation confirmed the reported connector with `waypoints=[]` is invalid while the corrected dogleg waypoints are valid.
2026-05-19 10:02 CST: simplified the board connector floating menu by removing the `直线` and `多段折线` route-mode actions. The menu now only exposes `直角折线` and `圆角折线`, while existing `straight`/manual `polyline` connector data remains supported for backward compatibility and internal editing flows. Verification: `apps/web npm run build` => passed; `apps/api .venv/bin/pytest tests/test_documents_api.py -k board -q` => passed.
2026-05-19 11:32 CST: fixed CloudDoc rich-text paste and read-mode cross-block selection. Root cause for paste loss: HTML clipboard parsing only converted top-level elements, so nested `div/p/h*` structures from formatted sources collapsed into one paragraph. `parsePastedHtmlToBlocks` now recursively extracts nested block elements, preserves headings/lists/quotes/code blocks, and splits paragraph text that contains newlines into separate document blocks. Root cause for cross-block selection failure: read mode still rendered each text block as a separate `<textarea>`, and native browser selection cannot span independent textarea controls. Read-only text blocks now render a normal selectable DOM text layer with the same visual text/comment highlights, while edit mode keeps textarea editing. Verification: `apps/web npm run build` => passed; `git diff --check` => passed; Playwright with local Chrome verified nested HTML paste creates separate `一级标题/第一段/第二段` blocks and read-mode cross-block selection returns `第一块内容\n第二块内容`. Restored local services after the abnormal restart: API `/health` returns ok and Web `/documents` returns 200.
2026-05-19 15:16 CST: fixed board floating sub-menu positioning. Root cause: toolbar submenus were rendered as `absolute left-0` inside the whole floating toolbar, so every submenu opened from the toolbar's left edge instead of following the hovered/clicked tool button. The board toolbar now records the triggering button's viewport rectangle, renders the submenu with fixed positioning anchored to that button, and clamps it within the browser viewport with above/below fallback. Verification: `apps/web npm run build` => passed; `git diff --check` => passed; Playwright with local Chrome opened a temporary board, selected a shape, hovered `文本样式`, and confirmed the submenu center aligned with the trigger button center while staying inside the viewport. Local services remain healthy: API `/health` returns ok and Web `/documents` returns 200.
2026-05-19 15:34 CST: fixed the board floating submenu vertical flip regression. Root cause: when there was not enough room below the toolbar, the submenu used the entire available space above the button as its placement height, so lower-page elements could push the menu to the page top. The placement logic now positions the submenu using its preferred panel height and only compresses max-height when needed, keeping the menu near the triggering button. Verification: `apps/web npm run build` => passed; `git diff --check` => passed; Playwright with local Chrome selected a near-bottom shape, hovered `文本样式`, and confirmed the menu opened just above the button (`verticalGap=246`) instead of jumping to the page top. Local services remain healthy: API `/health` returns ok and Web `/documents` returns 200.
2026-05-19 17:03 CST: moved board save/status notifications to the top-right corner and added subtle tone-based styling. Existing notice messages are now classified at render time as success, error, warning, or info, using soft emerald/rose/amber/sky palettes. `已保存` appears as a soft green success toast; save failures and permission/clipboard failures use soft red; local/offline draft and selection guidance use soft amber. Verification: `apps/web npm run build` => passed; `git diff --check` => passed; Playwright with local Chrome edited a temporary board, clicked 保存, and confirmed the `已保存` status appeared at the top-right with the emerald success style. Local services remain healthy: API `/health` returns ok and Web `/documents` returns 200.
2026-05-19 17:10 CST: added auto-close countdown behavior to board status notifications. Every new notice now starts a 5-second countdown, displays the remaining seconds beside the message, and clears itself automatically when the countdown reaches zero. Verification: `apps/web npm run build` => passed; `git diff --check` => passed; Playwright with local Chrome saved a temporary board, confirmed the `已保存` toast displayed a `5s` countdown, waited for the countdown to expire, and confirmed the toast was removed. Local services remain healthy: API `/health` returns ok and Web `/documents` returns 200.
2026-05-19 17:31 CST: implemented smart paste for board table shapes. When a table cell/row/column is selected, paste now accepts Excel/HTML tables, TSV text, and CSV text, parses the clipboard into a matrix, writes from the selected start cell, and automatically expands rows/columns when the pasted matrix exceeds the current table size. The board container is focused when table cells are selected and also handles native paste events, so direct paste from Excel/CSV sources is more reliable than the previous keyboard-only clipboard read path. Verification: `apps/web npm run build` => passed; `git diff --check` => passed; Playwright red check first confirmed a 2x2 TSV paste into the bottom-right of a 3x3 table did not expand; after implementation the same scenario saved as a 4x4 table with `A/B/C/D` in the expanded cells. A second Playwright case confirmed CSV quoted values such as `"a,b",c` parse correctly. Local services remain healthy: API `/health` returns ok and Web `/documents` returns 200.
2026-05-20 11:29 CST: fixed board floating color/menu panel placement after the latest positioning regression. Root cause: all toolbar panels shared a 240px preferred height, so compact panels such as color swatches were flipped upward as if they were much taller than their rendered size, leaving a large visual gap from the triggering button. The placement logic now uses panel-specific preferred heights for color swatches, shape palette, text style, line settings, and more menus. Verification: `apps/web npm run build` => passed; `git diff --check` => passed; Playwright with local Chrome selected a board shape, hovered `文字颜色`, and confirmed the color panel was centered on the trigger button with a 6px vertical gap instead of floating near the page top. Local services remain healthy: API `/health` returns ok and Web `/documents` returns 200.
2026-05-20 15:52 CST: redesigned the board selected-node floating text-style panel based on the Feishu-style reference screenshot. The `文本样式` popup is now a compact icon grid with text type/list/quote placeholders, real bold toggle, text decoration/link placeholders, real left/center/right alignment controls, vertical-alignment placeholders, and a rotate-text placeholder. Removed the global `min-w-48` constraint from floating toolbar panels so compact panels can use their panel-specific width instead of being stretched. Verification: red Playwright check first failed because `button[title="加粗"]` did not exist; after implementation `apps/web npm run build` => passed; `apps/api .venv/bin/pytest tests/test_documents_api.py -k board -q` => passed; Playwright with local Chrome opened a temporary board, selected a shape, opened `文本样式`, confirmed compact panel size `178x202`, clicked `加粗` and `右对齐`, saved, and verified persisted node style `fontWeight=600` and `textAlign=right`. `git diff --check` passed. Local services remain healthy: API `/health` returns ok and Web `/documents` returns 200.
2026-05-20 16:09 CST: refined the board text-style popup icon visuals after the user pointed out the character-based icons looked too rough. Replaced the ad-hoc text glyphs in the popup (`1≡`, `•≡`, quote characters, link emoji, arrows, alignment glyphs) with dedicated inline SVG line icons for paragraph, ordered list, bullet list, quote, bold, strikethrough, underline, italic, link, horizontal alignment, vertical alignment, and rotate text. Also replaced the main toolbar `文本样式` trigger with a matching line icon. Verification: `apps/web npm run build` => passed; `apps/api .venv/bin/pytest tests/test_documents_api.py -k board -q` => passed; Playwright with local Chrome reopened the temporary text-style toolbar flow and verified compact panel size `178x202`, real `加粗` and `右对齐` still persist as `fontWeight=600` and `textAlign=right`; `git diff --check` passed. Local services remain healthy: API `/health` returns ok and Web `/documents` returns 200.
2026-05-20 16:42 CST: implemented real behavior for all buttons in the board selected-node `文本样式` popup instead of showing placeholder notices. Board node/table-cell text style now persists and renders `textRole` (`paragraph`, `ordered_list`, `bullet_list`, `quote`), `fontStyle`, `textDecoration`, `linkUrl`, `verticalAlign`, and `textRotation` in addition to the existing font weight and horizontal alignment. Ordered/bullet/quote buttons alter displayed text formatting, strikethrough/underline/italic toggle text style, link prompts for a URL and normalizes bare domains to `https://`, vertical alignment changes text placement inside the shape/cell, and rotate text cycles by 90 degrees. Verification: `apps/web npm run build` => passed; Playwright with local Chrome clicked every text-style popup button on a temporary board node, saved, and verified persisted style `textRole=quote`, `fontWeight=600`, `fontStyle=italic`, `textDecoration=underline line-through`, `textAlign=right`, `verticalAlign=bottom`, `linkUrl=https://example.com/path`, and `textRotation=90`; `apps/api .venv/bin/pytest tests/test_documents_api.py -k board -q` => passed; `git diff --check` passed. Local services remain healthy: API `/health` returns ok and Web `/documents` returns 200.
2026-05-21 10:08 CST: removed the three duplicate horizontal alignment buttons from the board selected-node main floating toolbar. Alignment remains available in the `文本样式` popup, so the toolbar no longer shows two separate left/center/right alignment control groups. Verification: `apps/web npm run build` => passed; `apps/api .venv/bin/pytest tests/test_documents_api.py -k board -q` => passed; Playwright with local Chrome reran the full text-style popup flow and verified `右对齐` still persists through the popup; `git diff --check` passed. Local services remain healthy: API `/health` returns ok and Web `/documents` returns 200.
2026-05-21 11:14 CST: fixed the board text-role edit-mode mismatch reported by the user. Root cause: read mode rendered `textRole` prefixes for ordered list, bullet list, and quote, but double-click edit mode used a plain textarea with raw text only, so the visual role appeared to disappear. Edit mode now keeps textarea content as raw text for safe persistence, while rendering a non-editable marker gutter for ordered-list numbers, bullet dots, or quote markers beside the textarea; quote edit mode also keeps the quote styling. Verification: `apps/web npm run build` => passed; Playwright with local Chrome opened a temporary board node with `textRole=ordered_list`, double-clicked into edit mode, confirmed markers `1.` and `2.` remained visible, and confirmed the textarea value stayed raw as `vector\\nsearch`; `apps/api .venv/bin/pytest tests/test_documents_api.py -k board -q` => passed; `git diff --check` passed. Local services remain healthy: API `/health` returns ok and Web `/documents` returns 200.
2026-05-21 16:40 CST: improved the board ordered-list/quote visual layout after the user reported the centered list rendering looked poor. Root cause: the previous renderer prepended `1.` / `•` / quote markers into plain text and then used the generic centered SVG text flow, so list markers and content were treated as one centered text run and blank lines could produce misleading numbering. Text roles now use a dedicated row layout in read mode: marker column on the left, content column on the right, blank lines do not show markers, and quote/list blocks are rendered through `foreignObject` instead of generic SVG tspans. Edit mode also left-aligns textarea content when a marker gutter is active. Verification: `apps/web npm run build` => passed; Playwright with local Chrome verified `1.` appears left of `vect`, the content column aligns cleanly, and a blank middle line does not show a `2.` marker; the edit-mode marker test still passed; `apps/api .venv/bin/pytest tests/test_documents_api.py -k board -q` => passed; `git diff --check` passed. Local services remain healthy: API `/health` returns ok and Web `/documents` returns 200.
2026-05-22 15:42 CST: fixed the board top-bar share button and checked the ordinary cloud document share entry. Root cause: the board top bar rendered a static `分享` button with no click handler, while ordinary docs and folder tree actions already use the shared `DocumentShareDialog`. Board documents now reuse `DocumentShareDialog` with the share tab opened by default, gated by the same `canManage && canShare && !isSharedView` rule, and updates local document visibility after saving. Verification: red Playwright check first reproduced that clicking board `分享` did not open `权限与分享`; after the fix Playwright confirmed board `分享` opens the share dialog, and a separate ordinary doc Playwright check confirmed `权限/分享` still opens the same dialog. `apps/web npm run build` initially hit a stale Next `.next` chunk error (`Cannot find module './548.js'`), passed after clearing `.next`; `apps/api .venv/bin/pytest tests/test_documents_api.py -k \"board or share\" -q` => 3 passed; `git diff --check` passed. Local services remain healthy: API `/health` returns ok and Web `/documents` returns 200.
2026-05-22 16:18 CST: fixed the board save/submit UX regression where an in-flight save could make the canvas visually rebound while the user was panning or otherwise editing. Root cause: `saveBoard()` captured a request snapshot and unconditionally wrote that stale `boardToSave` back into live React state after the async API call completed. If the user moved the viewport or changed elements before the response returned, the older snapshot overwrote the newer local interaction state. The save path now tracks the latest live board with a ref, compares the live board signature against the submitted snapshot after the API returns, and only writes the submitted snapshot back when no newer local changes exist; otherwise it keeps the current canvas state dirty so the next autosave submits the newer state. Verification: red static regression check `/tmp/test-board-save-no-stale-overwrite.js` first failed on unconditional `setBoard(boardToSave)`, then passed after the guarded reconciliation change; `apps/web npm run build` => passed; `apps/api .venv/bin/pytest tests/test_documents_api.py -k board -q` => 1 passed.
2026-05-22 16:42 CST: fixed the board shape long-text overflow bug where scrolling to the top still could not reveal the first lines. Root cause: overflowing node text used a `foreignObject` scroll container but inherited the node vertical alignment (`middle`/`bottom`) through flex `align-items`; when content was taller than the shape, the flex layout clipped the beginning of the content outside the scrollable top. Both paragraph overflow text and list/quote role text now force top alignment while overflowing, preserving normal vertical alignment only when content fits. Verification: red static regression check `/tmp/test-board-overflow-align-start.js` first failed, then passed after the fix; `apps/web npm run build` => passed; `apps/api .venv/bin/pytest tests/test_documents_api.py -k board -q` => 1 passed; `git diff --check` => passed.
2026-06-02 15:58 CST: optimized CloudDoc ordinary-document code blocks. Code blocks now render as an independent block surface with a compact header, collapse toggle, searchable language selector, auto-wrap switch, copy action, and line-number gutter. Supported language/data-format options include Plain Text, HTTP, JavaScript/TypeScript, Python, Java, Go, Rust, C/C++/C#, PHP, Ruby, Swift, Kotlin, SQL, HTML/CSS, JSON/YAML/XML, Markdown, TOML, CSV, GraphQL, Dockerfile, Makefile, Regex, LaTeX, Lua, Julia, Haskell, Lisp, and MATLAB. Code-block Enter now inserts a code line, Tab inserts code indentation instead of changing document-block indent, slash commands are disabled inside code blocks, and pasted multi-line text remains in the current code block. The document content model now persists `language`, `wrap`, and `collapsed` code-block attributes. Documentation sync scope: updated local `已归档/cloud-doc-feature-ui-design.md` and synchronized it to online CloudDoc document `348e059b-7456-4898-b508-6056f9c7418f`; synchronized this progress log to online document `631d841f-25d8-4093-a1f5-46c985b6e506`. Verification: `git diff --check` => passed; `apps/web npm run build` => passed; 3100 restarted with `--hostname 0.0.0.0 --port 3100`; `curl -I http://127.0.0.1:3100/documents` => 200; browser opened `/documents` successfully but was unauthenticated, so direct editor UI creation was not available in that session.
2026-06-02 16:18 CST: adjusted the ordinary-document block action menu quick-type button layout. Root cause: the menu-top quick type buttons used a loose flex-wrap layout inside the compact block action menu, so the current type button such as `H3` could visually sit too close to the document content edge and look like it was floating over the text block. The text selection `评论/取消` toolbar and the left block handle position were restored unchanged after confirming they were not the target controls. The action menu is now slightly narrower and its quick type buttons use a fixed 8-column grid with contained selected-state styling, keeping all type buttons inside the menu panel. Verification pending: `git diff --check`, `apps/web npm run build`, and local 3100 response check.
2026-06-02 16:24 CST: raised the ordinary-document block action menu stacking layer. Root cause: the menu was rendered as a fixed floating panel but only used `z-30`, which could let selected document content or editor overlays visually cover the menu-top quick type button. The menu now uses `z-[80]` so its buttons render in front of the document body. Verification: `git diff --check` => passed; `apps/web npm run build` => passed; local 3100 restart and response check pending.
2026-06-02 16:32 CST: fixed the remaining block action menu quick-type button clipping. Root cause: the menu still could be overlapped by higher page chrome and its own container used `overflow-hidden`, so a selected quick-type button could look partially covered near the document edge. The menu now uses `z-[300]` and no longer clips overflow at the outer container. Verification: `git diff --check` => passed; `apps/web npm run build` => passed; local 3100 restart and response check pending.
2026-06-02 16:40 CST: corrected the actual ordinary-document block handle overlap issue after the user clarified the target control. Root cause: the `H3` button shown in the screenshot is the left block handle itself, not the action menu; the handle used a negative left offset with only `z-10`, so it could be visually covered by adjacent document content or page columns. The handle now uses a higher stacking layer (`z-[320]`) and a larger left offset (`left-[-54px]`) so it renders in front and stays separated from the title/body text. Verification: `git diff --check` => passed; `apps/web npm run build` => passed; local 3100 restart and response check pending.
2026-06-02 16:52 CST: fixed the block handle being covered by the left document outline column after inspecting the surrounding layout. Root cause: the ordinary document page is a grid with a left outline `<aside>` and a scrollable document `<section>`; the block handle intentionally uses negative left offset from the editor block, so it can cross into the left outline column's paint area. The left outline now has an explicit lower stacking layer (`z-0`) and the document scroll section uses `relative z-10`, allowing the block handle to render above the outline column instead of being visually clipped/covered at the column boundary. Verification: `git diff --check` => passed; `apps/web npm run build` => passed; local 3100 restart and response check pending.
2026-06-02 16:58 CST: moved ordinary-document content rightward on wide screens to reserve a visible block-handle gutter. Root cause: even with corrected stacking, the block handle used negative left offset from the content block and could sit too close to the left outline/document boundary. The document article now adds extra left padding on `xl` screens (`xl:pl-20`), keeping the handle fully inside the white document canvas while shifting content slightly right. Verification pending: `git diff --check`, `apps/web npm run build`, and local 3100 response check.
2026-06-02 17:03 CST: reduced the ordinary-document block-handle gutter after the user reported the content moved too far right. The wide-screen article left padding was changed from `xl:pl-20` to `xl:pl-14`, preserving enough room for the block menu button while reducing unnecessary blank space before the document content. Verification: `git diff --check` => passed; `apps/web npm run build` => passed; local 3100 restart and response check pending.
2026-06-02 17:08 CST: aligned the ordinary-document block menu handle with the top of each document block. Root cause: the handle still used vertical centering (`top-1/2 -translate-y-1/2`), which makes it hard to find on long text/code blocks. The handle now uses top alignment (`top-1`) while preserving the left gutter and high stacking layer, so it stays visible at the start of long blocks. Verification pending: `git diff --check`, `apps/web npm run build`, and local 3100 response check.
2026-06-02 17:16 CST: implemented resizable ordinary-document code blocks based on `/Users/yys235/Library/Containers/com.bytedance.macos.feishu/Data/Library/Application Support/LarkShell/screenshot/20260602164040.mp4`. Code blocks now store `height`, render the code body as a fixed-height internal scroll area, and expose a bottom drag handle for vertical resize with a 120px minimum and 1200px maximum. Long code no longer expands the full document height; line numbers and code content scroll together inside the code block. Updated local UI documentation and synchronized it to online CloudDoc. Verification pending: `git diff --check`, `apps/web npm run build`, and local 3100 response check.
2026-06-02 17:22 CST: fixed left page-outline item overflow for long headings. Outline entries now render as a single line with `truncate whitespace-nowrap`, so long headings are clipped with ellipsis instead of wrapping into multiple lines or showing long separator-like overflow. Verification: `git diff --check` => passed; `apps/web npm run build` => passed; local 3100 restart and response check pending.
2026-06-03 00:09 CST: added hover full-text hints for truncated cloud-document menu text. Page-outline entries, top breadcrumb folder/document labels, and compact summary chips now keep their single-line truncated layout but expose the full text via `title` on hover. Verification: `git diff --check` => passed; `apps/web npm run build` => passed; local 3100 restart and response check pending.
2026-06-09 18:10 CST: implemented the first storage-provider abstraction and board shape image/text content support. Backend uploads now route through `storage_service`: default `local` keeps the existing `uploads/` behavior, while `STORAGE_PROVIDER=s3` can write assets to an S3-compatible provider such as MinIO/S3-compatible OSS/COS when `boto3` is installed in the runtime. Board nodes now support an `image` field, render the image inside non-table shapes above the existing text area, expose a selected-shape toolbar image upload/replace action, allow image paste into a selected shape, and allow image removal from the shape more menu. Updated local README and content-model documentation, then synchronized online CloudDoc documents `CloudDoc` (`672c8bc1-2272-4834-8d0f-7ede79d232e5`, v18), `云文档内容模型设计` (`cbf9cc92-3309-4f5b-a2d7-49e142cae06f`, v4), and this progress document (`631d841f-25d8-4093-a1f5-46c985b6e506`, v223). Verification: `apps/api uv run pytest tests/test_documents_api.py::test_upload_image_asset -q` => passed; `apps/web npm run build` => passed; `git diff --check` => passed. Note: `uv lock` was attempted twice but PyPI connections reset before metadata download, so the optional S3 runtime dependency was documented instead of locked in this turn.
2026-06-09 18:50 CST: implemented 30-day permanent deletion for trashed documents. Documents now persist `deleted_at` when soft-deleted and clear it when restored; API startup ensures the runtime schema has the new column and starts an hourly purge worker. The purge removes documents whose `deleted_at` is older than `DELETED_DOCUMENT_RETENTION_DAYS` (default 30), deletes related comments, notifications, events, pins, shortcuts, favorites, permissions, share links, versions, and contents, and attempts to delete uploaded assets only when no other document version still references the same URL. Local README and content-model docs were updated and synchronized to online CloudDoc documents `CloudDoc` (`672c8bc1-2272-4834-8d0f-7ede79d232e5`, v19), `云文档内容模型设计` (`cbf9cc92-3309-4f5b-a2d7-49e142cae06f`, v5), and this progress document (`631d841f-25d8-4093-a1f5-46c985b6e506`, v225). Verification: `apps/api uv run pytest tests/test_documents_api.py::test_soft_delete_and_restore_document tests/test_documents_api.py::test_recently_deleted_document_is_not_purged_before_retention tests/test_documents_api.py::test_expired_deleted_document_is_purged_with_unreferenced_upload -q` => passed; `apps/api uv run pytest tests/test_documents_api.py -k "soft_delete or purge or upload_image or upload_pdf" -q` => 5 passed; `git diff --check` => passed; API `/health` => ok.
2026-06-09 19:18 CST: added content-version-level asset reference tracking and garbage collection. New `document_asset_refs` stores extracted resource URLs per `DocumentContent` version, including ordinary document images/PDF URLs and board shape `node.image.src` paths. Content saves, PDF uploads, DOCX imports, and duplicated documents now synchronize refs after content creation. The hourly cleanup now also runs `purge_unreferenced_document_assets`: current active document versions are always protected, history within `DELETED_DOCUMENT_RETENTION_DAYS` is protected, and all refs inside not-yet-expired trash documents are protected; assets outside those windows with no protected refs are deleted and their stale ref rows removed. Added regression coverage for board shape image deletion: deleting a shape removes the image from the current version, keeps the file during the history window, and deletes it after the window when no protected refs remain. Updated local README/content-model docs and synchronized online CloudDoc documents `CloudDoc` (`672c8bc1-2272-4834-8d0f-7ede79d232e5`, v20), `云文档内容模型设计` (`cbf9cc92-3309-4f5b-a2d7-49e142cae06f`, v6), and this progress document (`631d841f-25d8-4093-a1f5-46c985b6e506`, v227). Verification: `apps/api uv run pytest tests/test_documents_api.py::test_board_shape_image_ref_is_collected_and_garbage_collected_after_history_window tests/test_documents_api.py::test_expired_deleted_document_is_purged_with_unreferenced_upload -q` => passed; `apps/api uv run pytest tests/test_documents_api.py -k "soft_delete or purge or upload_image or upload_pdf or board_shape_image_ref" -q` => 6 passed; `apps/web npm run build` => passed; `git diff --check` => passed.
2026-06-09 19:36 CST: implemented board internal clipboard copy/paste for multi-selected entities. `Cmd/Ctrl+C` now captures selected board nodes into an internal clipboard without writing to the system text clipboard; selected connectors are copied only when the connector itself is selected and both endpoint nodes are included in the copied node set. `Cmd/Ctrl+V` pastes the copied group around the latest mouse position on the canvas, remaps node/connector IDs, offsets connector waypoints with the pasted group, and automatically selects the newly created nodes/connectors so the group can be dragged immediately. Text editing and table-cell editing keep their existing native/table copy-paste behavior. Verification: `apps/web npm run build` => passed; `git diff --check` => passed; restarted 3100 with `--hostname 0.0.0.0 --port 3100`.
2026-06-09 20:03 CST: constrained the Team Spaces directory tree height so large spaces no longer stretch the whole `/spaces` page. Each space card now keeps its header and enter action visible while the folder/document tree scrolls inside the card (`max-h-[420px]` with internal vertical overflow), reducing long-page scrolling when a team space contains many documents. Verification: `apps/web npm run build` => passed; `git diff --check` => passed.
2026-06-09 20:43 CST: completed the Team Spaces long-page fix after the first directory-only change did not affect the user's visible top area. Root cause: `/spaces` can also be stretched by the Organization Management member list and current-session list before the directory section, and the directory card needed explicit overflow clipping. Member management now scrolls internally after `max-h-80`, current sessions scroll after `max-h-64`, and directory cards use `overflow-hidden` so their tree panes scroll instead of painting outside the card. Verification: `apps/web npm run build` => passed; `git diff --check` => passed; restarted 3100 with `--hostname 0.0.0.0 --port 3100`; `curl -I http://127.0.0.1:3100/spaces` => 200.
2026-06-09 20:52 CST: added Team Space rename support and clarified the current-session panel semantics. Backend now exposes `PATCH /api/spaces/{space_id}` with manage-space permission checks, blank-name validation, and 120-character limit. The `/spaces` Team Spaces list now provides an inline `改名` action with Enter-to-save and Esc/cancel behavior, then refreshes the server-rendered page after save. `当前会话` remains the account login-session/device list used for revoking active browser/device sessions. Verification: `apps/api uv run pytest tests/test_documents_api.py -k "space_rename" -q` => 2 passed; `apps/api uv run pytest tests/test_documents_api.py::test_space_owner_can_rename_space -q` => passed; `apps/web npm run build` => passed; `git diff --check` => passed; restarted 3100 with `--hostname 0.0.0.0 --port 3100`; `curl -I http://127.0.0.1:3100/spaces` => 200; API `/health` => ok.
2026-06-10 16:18 CST: wrote the PRD for board shape rich mixed text/image content. Added local document `已归档/board-shape-rich-content-prd.md`, covering the current `text + image` implementation gap, alternatives comparison, recommended lightweight content-block model, data migration/compatibility, editing interaction, image resizing/sorting, internal scrolling, copy/paste, undo/redo, save, asset-reference collection, AI/MCP output, table-shape boundary, implementation phases, test plan, and risk controls. Created the matching online CloudDoc PRD `CloudDoc 画板形状图文混排能力 PRD` (`42d5d4a3-5744-4dd6-8934-71d96004ee95`, v1) in the existing画板 PRD folder. Verification: PRD placeholder scan found no `TODO/TBD/待定`; `git diff --check` => passed.
2026-06-10 10:49 CST: implemented the first-stage board shape rich mixed text/image content feature from `已归档/board-shape-rich-content-prd.md`. Ordinary board nodes now support `content` blocks (`paragraph` and `image`) while preserving legacy `text` and single `image` compatibility. Existing `text + image` nodes migrate at runtime into ordered content blocks; new saves use board `version: 3`, keep `text` as a paragraph summary, and include current in-progress rich-content edits in manual save and offline draft persistence. Shape content now renders as an internal scrolling block flow, supports double-click editing, Enter-to-add-paragraph, empty-paragraph Backspace delete, Esc/canvas-click commit, image upload/paste insertion, image block up/down/delete actions, and drag-resize via a bottom-right image handle. Table nodes remain on their existing table model. Backend regression coverage now verifies asset references are collected from `nodes[*].content[*].src`. Verification: `apps/api uv run pytest tests/test_documents_api.py::test_board_shape_rich_content_image_refs_are_collected -q` => passed; `apps/api uv run pytest tests/test_documents_api.py -k "board_shape_image_ref or rich_content_image_refs or upload_image or board" -q` => 4 passed; `apps/web npm run build` => passed; `git diff --check` => passed. Local service restart and online document sync pending in this turn.
2026-06-10 13:49 CST: completed the follow-up drag interaction for board shape rich-content image blocks. Image blocks are now draggable while the shape is in rich-content editing mode; dropping over another image or paragraph reorders the dragged image block before/after that target. Drag sorting is isolated to the internal content editor, while the image resize handle keeps using its own pointer flow and the outer board node drag remains disabled during content editing. Updated the rich-content PRD implementation-status section. Verification: `apps/web npm run build` => passed; `git diff --check` => passed.
2026-06-10 17:38 CST: cancelled board shape image/rich mixed text-image support and restored ordinary board shapes to text-only behavior. Removed the frontend board shape image upload/paste/rendering and rich-content editing paths, removed the board shape image/rich-content regression tests, restored board default/save content version to `version: 2`, and restored backend default board content to `version: 2`. Ordinary cloud-document image upload/preview/object-storage behavior is unchanged. Updated `已归档/board-shape-rich-content-prd.md` to mark the PRD as cancelled and retained only as archive reference. Verification: board shape image/rich-content residual scan in board frontend/API test/default-board files found no code matches; `apps/api uv run pytest tests/test_documents_api.py -k "upload_image or upload_pdf or purge" -q` => 4 passed; `apps/web npm run build` => passed.
2026-06-23 10:25 CST: fixed the board shape text-edit switching regression reported with `/Users/yys235/Library/Containers/com.bytedance.macos.feishu/Data/Library/Application Support/LarkShell/screenshot/20260623100654.mp4`. Root cause: shape text editing kept the in-progress value only in `editingText`; switching directly from one shape's edit mode to another shape could replace the active edit target before the previous value was committed, and stale blur handlers could clear the new edit session. The board editor now tracks active text edit target/value in refs, commits the previous node before starting a new node edit, scopes blur/Escape commits to the textarea's own node id, and includes active edit text in manual/autosave payload reconciliation. Verification: Playwright regression `/tmp/repro-board-edit-switch.js` created a temporary board, edited node A, directly double-clicked node B, and confirmed node A still rendered `第一个节点新内容`; `apps/web npm run build` => passed; `apps/api .venv/bin/pytest tests/test_documents_api.py -k board -q` => 1 passed; `git diff --check` => passed.
2026-06-23 10:44 CST: added periodic board shape text-edit commits to reduce data-loss risk while a user stays inside shape edit mode. Active shape textarea edits are now merged into board state every 2 seconds without adding undo-history entries, marked dirty for the existing autosave/offline-draft pipeline, and final blur/switch commits skip duplicate history writes when the periodic commit already applied the same text. Verification: red static check `/tmp/test-board-text-periodic-commit.js` first failed before implementation and passes after the change; Playwright regression `/tmp/test-board-periodic-edit-save.js` created a temporary board, entered shape edit mode, filled `定时提交内容`, stayed in edit mode, waited for periodic commit/autosave, and confirmed the backend document node text was updated; `apps/web npm run build` => passed; `apps/api .venv/bin/pytest tests/test_documents_api.py -k board -q` => 1 passed; `git diff --check` => passed.
2026-06-23 10:58 CST: corrected the board shape timed-commit behavior to preserve the original final commit path. Periodic shape text commits now remain a safety pre-merge into local board state only; blur, Escape, canvas click, and switching to another shape still call the original `commitTextEditToBoard` path so existing final-submit semantics, history behavior, and save pipeline are not replaced by the timer. Verification: static regression `/tmp/test-board-final-commit-preserved.js` confirmed `finishTextEditing` always calls `commitTextEditToBoard(targetId, text)` and does not conditionally skip based on the current node text; `/tmp/test-board-text-periodic-commit.js` confirmed the timer still uses `setBoard` rather than `commitBoard`; Playwright regression `/tmp/repro-board-edit-switch.js` confirmed editing node A and directly double-clicking node B preserves node A text `第一个节点新内容`; `apps/web npm run build` => passed; `apps/api .venv/bin/pytest tests/test_documents_api.py -k board -q` => 1 passed.
2026-06-23 12:16 CST: implemented dedicated board Code Node support from `docs/superpowers/specs/2026-06-23-board-code-node-design.md`. The board shape palette now includes a `code` node type with dedicated default size, normalization fields (`codeLanguage`, `codeWrap`, `codeCollapsed`, `codeHeight`), a code-specific icon, monospace header/content rendering, line numbers, internal scrolling, double-click/Enter-style text editing via the existing board edit commit pipeline, Tab/Shift+Tab indentation inside the textarea, language selection, wrap toggle, copy-code action, collapse/expand, and code-specific toolbar behavior that hides normal text formatting controls. Ordinary shapes remain text-only, while code nodes still participate in selection, dragging, resizing, quick-add, connectors, history, autosave/offline draft, and internal node copy/paste. Verification: `apps/web npm run build` => passed; `git diff --check` => passed.
2026-06-23 15:37 CST: fixed Code Node board save failure reported as `保存失败：连接线数据无效，已尝试自动修正后请重试`. Root cause was backend board validation not yet allowing the new `code` node type, so any saved board containing a Code Node was rejected before connector validation could succeed. Added `code` to backend `allowed_node_types` and covered a board payload containing a Code Node plus a connector in the existing board API regression test. Verification: `apps/api uv run pytest tests/test_documents_api.py -k board -q` => 1 passed; `apps/web npm run build` => passed; `git diff --check` => passed.
2026-07-15 15:07 CST: stabilized board connectors while shapes are dragged and corrected arrow placement at node boundaries. Drag start now snapshots resolved waypoints for both persisted and implicit auto-routed connectors; each pointer move transforms that fixed route instead of re-running route candidate selection, and pointer release only reroutes a connector when the preserved path actually crosses a connected node. The SVG arrow reference point now aligns with the real arrow tip (`refX=9`) so the tip no longer extends inside the target shape. Updated `已归档/board-connector-editing-improvement-prd.md` with the behavior and regression scope. Verification: `apps/web npx tsc --noEmit` => passed; `apps/web npm run build` => passed; `git diff --check` => passed. Browser edit-mode regression remains pending because the available local browser sessions are not authenticated to CloudDoc.
2026-07-15 15:07 CST: synchronized the board connector drag-stability records to online CloudDoc under the existing `clouddoc` root. Sync scope: `已归档/board-connector-editing-improvement-prd.md` -> `490e0113-76d5-404d-bbe5-2e3c9342d34d` (v33), `DEVELOPMENT_PROGRESS.md` -> `631d841f-25d8-4093-a1f5-46c985b6e506` (v240 before this sync-record writeback). Verification: fetched online document titles matched the expected PRD/progress titles, and online plain text contained the markers `第二十九轮形状拖动连接线稳定性` and `stabilized board connectors`.
2026-07-15 15:27 CST: completed authenticated browser regression for board connector drag stability and fixed two additional edge cases found during sampling. Auto-routed connectors that initially collapse to a horizontal/vertical line now expand into endpoint stubs plus a deterministic middle route for the drag snapshot. Collinear simplification now removes points only when both adjacent segments continue in the same direction, preserving reversed endpoint-protection stubs so arrows approach anchors from outside the target node. Browser regression used temporary board `pytest-board-connector-stability-*`: 7 samples covered an implicit route crossing its old bend, 4 samples covered an explicit multi-bend route, and 4 multi-selection samples confirmed 3 nodes plus 2 connectors moved together by `(+20,+15)` per stage. Manual save and reload preserved final node coordinates `drag-source=(260,240)`, `target-right=(495,380)`, `target-bottom=(635,380)`, valid connector paths, and arrow `refX=9`. Temporary board and dev session were cleaned up. Verification: `apps/web npx tsc --noEmit` => passed; `apps/web npm run build` => passed; `apps/api uv run pytest tests/test_documents_api.py -k board -q` => 1 passed, 30 deselected; `git diff --check` => passed; frontend `3100` and backend `8001` health checks => 200.
2026-07-15 15:33 CST: synchronized the final board connector drag-stability implementation and authenticated regression records to online CloudDoc under the existing `clouddoc` root. Sync scope: `已归档/board-connector-editing-improvement-prd.md` -> `490e0113-76d5-404d-bbe5-2e3c9342d34d` (v34), `DEVELOPMENT_PROGRESS.md` -> `631d841f-25d8-4093-a1f5-46c985b6e506` (v242 before this sync-record writeback). Verification resolved each document through its current version/content association and confirmed the online bodies contain `共线折点只在前后线段同方向时合并`, `自动直线连接线连续拖动采样 7 个中间位置`, `completed authenticated browser regression`, and `7 samples covered an implicit route`.
2026-07-16 11:28 CST: restarted the CloudDoc frontend and backend as detached, Codex-independent `screen` services. Recreated `clouddoc-web-3100` with `CLOUDDOC_BACKEND_ORIGIN=http://127.0.0.1:8001` and `next dev --hostname 0.0.0.0 --port 3100`, and recreated `clouddoc-api-8001` with `uv run uvicorn app.main:app --host 127.0.0.1 --port 8001 --reload`. Removed the generated frontend `.next` cache before startup to avoid stale/missing chunk failures. The detached screen servers remain alive when Codex closes or restarts; no other Node/Uvicorn services were stopped. Verification: both screen sessions are detached, frontend listener is `*:3100`, backend listener is `127.0.0.1:8001`, `/documents` => 200, proxied `/api/system/bootstrap/status` => 200, and backend `/health` => 200.
2026-07-16 11:28 CST: synchronized the Codex-independent frontend/backend restart record to online CloudDoc document `631d841f-25d8-4093-a1f5-46c985b6e506` under the existing `clouddoc` root (v244 before this sync-record writeback). Verification resolved the document through its current version/content association and confirmed the online body contains `restarted the CloudDoc frontend and backend as detached, Codex-independent`.
2026-07-17: fixed LAN access to the CloudDoc backend. Root cause was the API screen command binding Uvicorn to `127.0.0.1:8001`, which rejected requests from other PCs even though the frontend was already listening on `0.0.0.0:3100`. Recreated only `clouddoc-api-8001` with `--host 0.0.0.0`; CloudDoc frontend remains on `0.0.0.0:3100` and its proxy continues targeting `127.0.0.1:8001`. Verification from the host LAN address `192.168.123.173`: backend `/health` => 200, frontend `/documents` => 200, frontend proxied `/api/system/bootstrap/status` => 200; no other Node/Uvicorn service was stopped.
2026-07-17: synchronized the LAN backend bind fix to online CloudDoc progress document `631d841f-25d8-4093-a1f5-46c985b6e506` under the existing `clouddoc` root (v246 before this sync-record writeback). Verification resolved the document through its current version/content association and confirmed the online body contains `fixed LAN access to the CloudDoc backend`.
2026-08-03: deployed GitHub `main` commit `b64aadd` to the production server `192.168.123.233` from the previous production commit `a16bdb3`. Before deployment, backed up the PostgreSQL database, `.env`, and upload directories to `/home/qqqqqq/clouddoc-backups/20260803-172331` (database dump: 53 MB). Updated Python and Node dependencies, retained MCP on the compatible 1.x line (`mcp==1.29.0`), and pinned the repository dependency to `mcp>=1.0.0,<2.0.0` so future deployments cannot silently cross the major-version boundary. Completed the Next.js production build and restarted `clouddoc-web`, `clouddoc-api`, and `clouddoc-mcp`. Verification: all three systemd services are active and listening on `0.0.0.0:3100`, `0.0.0.0:8000`, and `0.0.0.0:8010`; LAN Web `/documents` => 200; API `/health` => 200; bootstrap status reports `database_ok=true`, `schema_ok=true`, and `uploads_ok=true`; MCP `/mcp` responds with the expected 406 for a plain GET without the Streamable HTTP accept headers.
2026-08-03: synchronized the production deployment record to online CloudDoc progress document `631d841f-25d8-4093-a1f5-46c985b6e506` under the existing `clouddoc` root. Verification read the document back through the CloudDoc MCP bridge and confirmed the online Markdown contains `deployed GitHub`, `b64aadd`, and `192.168.123.233`.
